...
class DHCP:
    ...
    :param eth: WIZNET Object.
    :param list mac_address: Hardware MAC.
    :param str hostname: The desired hostname.
    :param int response_timeout: DHCP Response timeout.
    :param bool debug: Enable debugging output.
    """

    def __init__(self, eth, mac_address, hostname=None, response_timeout=30, debug=True):
        self._debug = debug
        ...
        self.dhcp_server_ip = BROADCAST_SERVER_ADDR #255.255.255.255
        self.local_ip = 0
        self.gateway_ip = 0
        self.subnet_mask = 0
        self.dns_server_ip = 0

        #IP Leasing configuration
        self._lease_time = 0
        ...
        mac_string = "".join("{:02X}".format(o) for o in mac_address)
        self._hostname = bytes(hostname or "ArcelikNEXTDefault", "utf-8")
        if self._debug:
    ...
    def _dhcp_state_machine(self):
        ...
        if self._dhcp_state == STATE_DHCP_START:
            ...
            try:
                self._sock = socket.socket(type=socket.SOCK_DGRAM) #UDP
            except RuntimeError:
                if self._debug:
                ...
                if self._last_lease_time == 0 or time.time() > (self._last_lease_time + self._lease_time):
                    if self._debug:
                        pass
                    #     print("* DHCP: Send discover to {}".format(self.dhcp_server_ip))
                    self.send_dhcp_message(STATE_DHCP_DISCOVER, (time.time() - self._start_time))
                    self._dhcp_state = STATE_DHCP_DISCOVER
                else:
                    if self._debug:
                        pass
                    #     print("* DHCP: Send request to {}".format(self.dhcp_server_ip))
                    self.send_dhcp_message( DHCP_REQUEST, (time.time() - self._start_time), True)
