...
`wiznet5k`
================================================================================
Driver for WIZNET 5k ethernet modules.

Version(s) Log 
    Version : Release D : Author(s)
    -------------------------------
    1.0     : 03.2023   : Cagin Agirdemir (cagin.agirdemir@arcelik.com)
...
import time
import ethernet_dhcp as dhcp
import ethernet_tcp as tcp
...
DEFAULT_MAC = (0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xEF)
...
class WIZNET:
    ...
    def __init__(self, spi_bus, cs, reset=None, is_dhcp=True, mac=DEFAULT_MAC, hostname=None, dhcp_timeout=30, debug=True):
        self._debug = debug
        self._chip_type = None
        self._device = spi_bus #spi object
        self._device.init(baudrate=500000, polarity=0, phase=0, bits=8, firstbit=0, sck=Pin(18), mosi=Pin(23), miso=Pin(19)) #baudrate=500000
        self._cs = cs #chip select 
        #resest wiznet module prior to initialization
        if reset:
            reset.value(1)
            time.sleep(0.1)
            reset.value(0)
        ...
        self._ch_base_msb = 0
        assert self._w5500_init() == 1, "Failed to initiazlize WIZNET Module."
        # Set MAC address
        ...
        ret = self.set_tcp_socket()
        if ret != 1:
            self._tcp_socket = None
            raise RuntimeError("Failed to initalize TCP connection!")
        

    def set_tcp_socket(self):
        if self._debug:
            print("* Initializing TCP socket")
        self._tcp_socket = tcp.TCP_SOCK(self)
        ret = self._tcp_socket.tcp_connect()
        return 1
    ...
    def set_dhcp(self, hostname=None, response_timeout=30):
        ...
        # First, wait link status is on (CABLE CONTROL)
        # to avoid the code during DHCP - assert self.link_status, "Ethernet cable discoonected!"
        ...
        while 1:
            ...
            time.sleep(1)
            if self._debug:
                print("Check Ethernet Cable! Status:", self.link_status)
        if self._debug:
    ...
    def sendJson(self,data):
        self._tcp_socket.Send_Json(data)

    def sendString(self,data):
        self._tcp_socket.Send_String(data)  

    def getByteArrayInto(self):
        return self._tcp_socket.Get_ByteArray()

    def getString(self):
        return self._tcp_socket.Get_String() 
    ...
    def _w5500_init(self):
        ...
        if self.detect_w5500() == 1:
            ...
            for i in range(0, W5200_W5500_MAX_SOCK_NUM):
                ctrl_byte = 0x0C + (i << 5)
                self.write(0x1E, ctrl_byte, 2) #Receive Buffer Size 2 -> 2kb
                self.write(0x1F, ctrl_byte, 2) #Trasmitter Buffer Size 2 -> 2kb
        else:
            return 0
        return 1
    ...
    def sw_reset(self):
        ...
        self.write(REG_MR, 0x04, 0x80) #0x04 -> write mode control phase
        mode_reg = self.read(REG_MR, 0x00)
        if mode_reg[0] != 0x00:
            return -1
        return 0
    ...
    def socket_available(self, socket_num, sock_type=SNMR_TCP):
        ...
        assert socket_num <= 0x08, "Provided socket exceeds max_sockets."
    ...
    def socket_connect(self, socket_num, dest, port, conn_mode=SNMR_TCP):
        ...
        if conn_mode == SNMR_TCP:
            ...
            while self.socket_status(socket_num)[0] != SNSR_SOCK_ESTABLISHED:
                time.sleep(0.001)
                if self._debug:
                    print("Socket Status: %X" % self.socket_status(socket_num)[0])
    ...
    def _send_socket_cmd(self, socket, cmd):
        self._write_socket(socket, REG_SNCR, cmd)
        while self._read_socket(socket, REG_SNCR) != b"\x00":
            if self._debug:
    ...
    def socket_listen(self, socket_num, port, conn_mode=SNMR_TCP):
        ...
        assert self.link_status, "Ethernet cable disconnected!"
        if self._debug:
            print("* Listening on port={}, ip={}".format(port, self.pretty_ip(self.read(REG_SIPR, 0x00, 4))))
        # Initialize a socket and set the mode
        self.src_port = port
        res = self.socket_open(socket_num, conn_mode=conn_mode)
        self.src_port = 0
        ...
        while status[0] not in (SNSR_SOCK_LISTEN, SNSR_SOCK_ESTABLISHED, SNSR_SOCK_UDP):
            status = self._read_socket(socket_num, REG_SNSR)
    ...
    def socket_open(self, socket_num, conn_mode=SNMR_TCP):
        ...
        if status in (
            ...
            time.sleep(0.00025)
            self._write_socket(socket_num, REG_SNMR, conn_mode)
            self._write_socket(socket_num, REG_SNIR, 0xFF)
            if self.src_port > 0:
            ...
            # open socket
            self._write_socket(socket_num, REG_SNCR, CMD_SOCK_OPEN)
            return self._read_socket(socket_num, REG_SNCR)
            assert (self._read_socket(socket_num, REG_SNCR)[0] == 0x13
                or self._read_socket(socket_num, REG_SNCR)[0] == 0x22 ), "Could not open socket in TCP or UDP mode."
            return 0
        return 1
    ...
    def socket_read(self, socket_num, length):
        ...
        if ret == 0:
            # no data on socket?
            status = self._read_socket(socket_num, REG_SNMR)
        ...
        if ret > 0:
            ...
            self._write_socket(socket_num, REG_SNCR, CMD_SOCK_RECV)
            self._read_socket(socket_num, REG_SNCR)
        return ret, resp
    ...
    def _get_rx_rcv_size(self, sock):
        ...
        val = 0
        val_1 = self._read_socket(sock, REG_SNRX_RSR)
        val_1 += self._read_socket(sock, REG_SNRX_RSR + 1)
        while val != val_1:
            val_1 = self._read_socket(sock, REG_SNRX_RSR)
            val_1 += self._read_socket(sock, REG_SNRX_RSR + 1)
            if val_1 != 0:
                val = self._read_socket(sock, REG_SNRX_RSR)
                val += self._read_socket(sock, REG_SNRX_RSR + 1)
